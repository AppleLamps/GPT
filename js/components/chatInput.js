...
function observeInputContainerResize() {
    const inputContainerElement = document.querySelector('.input-container');
    if (!inputContainerElement) {
        console.error("Input container element not found for ResizeObserver.");
        return;
    }
    ...
    inputContainerResizeObserver = new ResizeObserver(entries => {
        for (let entry of entries) {
            // Use borderBoxSize for more accurate height including padding/border
    ...
    inputContainerResizeObserver.observe(inputContainerElement);
    console.log("ResizeObserver attached to input container.");

    // Initial update
    const initialHeight = inputContainerElement.getBoundingClientRect().height;
    if (initialHeight) {
        document.documentElement.style.setProperty('--dynamic-input-area-height', `${initialHeight}px`);
    }
}
...
/**
 * Updates the toolbar buttons based on current application state
 */
function updateToolbarButtons() {
    // Update all toolbar buttons using the existing model UI update function
    updateInputUIForModel(state.getActiveCustomGptConfig());
}
