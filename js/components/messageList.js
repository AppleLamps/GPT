...
export function addUserMessage(textContent, imageData = null) {
    ...
    let imageHtml = '';
    if (imageData) {
        // Add an image element if imageData is provided
        imageHtml = `<img src="${imageData}" class="history-image" alt="User Uploaded Image">`;
    }

    userMessage.innerHTML = `<div class="user-bubble">${imageHtml}${utils.escapeHTML(textContent)}</div>`;
...
export function renderMessagesFromHistory(history) {
    if (!messageContainerElement || !history?.length) return;
    clearMessageListUI();

    history.forEach(message => {
        if (message.role === 'user') {
            addUserMessage(message.content, message.imageData);
        } else if (message.role === 'assistant') {
            const aiMessageElement = createAIMessageContainer();
            if (aiMessageElement) {
                let finalHtmlContent;
                let rawContentForActions;
                let isImageMessage = false;

                if (message.imageUrl) {
                    // Handle image generation message
                    isImageMessage = true;
                    finalHtmlContent = `
                        <img src="${message.imageUrl}" alt="Generated Image" style="max-width: 100%; max-height: 400px; border-radius: 8px; display: block;">
                        ${message.content ? `<p style="font-size: 0.8em; color: var(--text-muted); margin-top: 8px;">${utils.escapeHTML(message.content)}</p>` : ''}
                    `;
                    rawContentForActions = message.imageUrl; // For copying the image URL
                } else {
                    // Handle regular text message
                    resetParser();
                    accumulateChunkAndGetEscaped(message.content || "");
                    finalHtmlContent = parseFinalHtml();
                    rawContentForActions = message.content || "";
                }

                finalizeAIMessageContent(aiMessageElement, finalHtmlContent);
                setupMessageActions(aiMessageElement, rawContentForActions, isImageMessage);
            }
        }
    });
