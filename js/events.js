...
// Add this function to initialize swipe detection
function initializeSwipeGestures() {
    let touchStartX = 0;
    const SWIPE_THRESHOLD = 100; // Minimum swipe distance in pixels
    const EDGE_THRESHOLD = 50;  // Distance from edge to start tracking swipe
    
    document.addEventListener('touchstart', (e) => {
        // Only track swipes that start near the edge of the screen
    ...
    document.addEventListener('touchend', (e) => {
        ...
        // If sidebar is hidden and swipe is right from left edge
        if (deltaX > SWIPE_THRESHOLD && touchStartX < EDGE_THRESHOLD) {
            ui.toggleSidebar(true); // Open sidebar
        } 
        // If sidebar is visible and swipe is left
        else if (deltaX < -SWIPE_THRESHOLD && ui.sidebar?.classList.contains('visible')) {
            ui.toggleSidebar(false); // Close sidebar
        }
