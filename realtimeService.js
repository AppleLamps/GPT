...
function updateState(updates) {
    ...
    if (updates.status !== undefined) setRealtimeSessionStatus(updates.status);
    if (updates.active !== undefined) setIsRealtimeSessionActive(updates.active);
    if (updates.connection !== undefined) setRealtimeConnection(updates.connection);
    if (updates.dataChannel !== undefined) setRealtimeDataChannel(updates.dataChannel);
    if (updates.remoteStream !== undefined) setRealtimeRemoteAudioStream(updates.remoteStream);
    if (updates.ephemeralKey !== undefined) setRealtimeEphemeralKey(updates.ephemeralKey);
    if (updates.transcript !== undefined) setCurrentRealtimeTranscript(updates.transcript);
    ...
    // Use getters for logging
    console.log("Realtime State Updated:", {
        status: getRealtimeSessionStatus(),
        active: getIsRealtimeSessionActive(),
        transcript: getCurrentRealtimeTranscript()
    });
}
...
function handleServerEvent(eventData) {
    ...
    switch (eventData.type) {
        ...
        case 'response.text.delta':
            ...
            if (eventData.delta) {
                updateState({ transcript: getCurrentRealtimeTranscript() + eventData.delta }); // <-- Use getter
            }
            break;
